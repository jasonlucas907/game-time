...
// decides to build hostess randomly
  randomHostessCreate() {
    let randomPosition = Math.floor(Math.random() * 700) + 1;
    let randomNumber = Math.floor(Math.random() * 25000) + 1;
...
// if still has lives reset invaders and continue
  collisionReset() {
    ...
    if (this.lives >= 1) {
      ...
      setTimeout(() => {this.player1.explode = 'no'; }, 1000);
...
//  if pass level 5 win the game
  levelUpEval()  {
    if (this.level <= 5) {
      ...
      this.removeGameMusic();
      this.winnerMessage();
      setTimeout(() => {this.winnerVideo(); }, 3000)
...
// //  display player level
  levelDisplay() {
    $('.level-display-txt').text('level ' +this.level)
  }
...
// remove game background audio
  ...
  winnerVideo() {
    $(".game-container").append(`
      ...
      `);
    $('.intro-video-container').delay(10000).fadeOut(1000, function(){
